...
export default class FileMatrix{
    ...
    getVizCode(){
        ...
        while (x_row){
            grafo += `Node${"row"+x_row.id.replace(".", "")}[label="${x_row.id}",fillcolor="plum"];\n`
            x_row = x_row.next
        }

        // conectando nodos de la fila cabecera

        grafo += `root -> Node${"row"+ this.rows.first.id.replace(".", "")} [dir=both];\n` // connecting root node to first row

        x_row = this.rows.first
        while (x_row.next){
            grafo += `Node${"row"+x_row.id.replace(".", "")} -> Node${"row"+ x_row.next.id.replace(".", "")} [dir=both];\n`
            x_row = x_row.next
        }
        ...
        if (this.columns.first){
            ...
            while (x_row){
                ...
                while (x_row2){
                    grafo += `Node${x_row2.x.replace(".", "")+x_row2.y}[label="${x_row2.permission}",fillcolor="plum"];\n`
                    x_row2= x_row2.right
                }
            ...
            x_row = this.rows.first
            while (x_row){
                grafo += "{rank=same;" + "Noderow"+ x_row.id.replace(".", "") + ";"
                let x_row2 = x_row.access
                while (x_row2){
                    grafo += `Node${x_row2.x.replace(".", "")+x_row2.y};`
                    x_row2= x_row2.right
                }
            ...
            while (x_row){
                ...
                while (x_row2){
                    if (x_row2.up instanceof HeaderNode){
                        grafo += `Node${x_row2.x.replace(".", "")+x_row2.y} -> Node${"col"+x_row2.y} [dir=both];\n`
                    }
                    else{
                        grafo += `Node${x_row2.x.replace(".", "")+x_row2.y} ->Node${x_row2.up.x.replace(".", "")+x_row2.up.y} [dir=both];\n`
            ...
            while (x_row){
                ...
                while (x_row2){
                    if (x_row2.left instanceof HeaderNode){
                        grafo += `Node${"row"+x_row2.left.id.replace(".", "")} -> Node${x_row2.x.replace(".", "")+x_row2.y} [dir=both];\n`
                    }
                    else{
                        grafo += `Node${x_row2.left.x.replace(".", "")+x_row2.left.y} -> Node${x_row2.x.replace(".", "")+x_row2.y} [dir=both];\n`
